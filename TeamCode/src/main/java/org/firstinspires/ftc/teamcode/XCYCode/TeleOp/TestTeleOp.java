package org.firstinspires.ftc.teamcode.XCYCode.TeleOp;


import com.acmerobotics.dashboard.config.Config;
import com.acmerobotics.roadrunner.geometry.Pose2d;
import com.qualcomm.robotcore.eventloop.opmode.LinearOpMode;
import com.qualcomm.robotcore.eventloop.opmode.TeleOp;
import com.qualcomm.robotcore.hardware.DcMotor;

import static org.firstinspires.ftc.teamcode.XCYCode.Configurations.*;

import org.firstinspires.ftc.teamcode.XCYCode.XCYBoolean;
import org.firstinspires.ftc.teamcode.XCYCode.XCYMecanumDrive;

@TeleOp(name = "testTeleOp")
@Config
public class TestTeleOp extends LinearOpMode {
   private XCYMecanumDrive drive;
   public static double lift_pow_ratio = -0.5;
   public static double spin_pow_ratio = 0.3;
   public static double intake_pow_ratio = 0.5;

   @Override
   public void runOpMode() {
      drive = new XCYMecanumDrive(hardwareMap);
      drive.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
      drive.getLocalizer().setPoseEstimate(get_pos_from_csv());
      waitForStart();
      XCYBoolean drive_auto_move = new XCYBoolean(() -> gamepad1.back);
      Pose2d drive_pos = new Pose2d();
      while (opModeIsActive()) {
         XCYBoolean.bulkRead();
         double x = -gamepad1.left_stick_y;
         double y = -gamepad1.right_stick_x;
         double turn_val = -gamepad1.right_trigger + gamepad1.left_trigger;
         Pose2d power = (new Pose2d(x, y, turn_val)).times(0.4);

         drive.setWeightedDrivePower(power);
         drive.update();

         double intake_move_power = gamepad2.left_stick_y * intake_pow_ratio;
         double lift_power = gamepad2.right_stick_y * lift_pow_ratio;
         double spin_base_power = (gamepad2.left_trigger - gamepad2.right_trigger) * spin_pow_ratio;

         if (gamepad1.start) {
            drive.setPoseEstimate(new Pose2d());
            drive.getLocalizer().setPoseEstimate(new Pose2d());
            drive_pos = drive.getPoseEstimate();
         }
         if (drive_auto_move.toTrue()) {
            drive.initSimpleMove(drive_pos);
            while (drive_auto_move.get()) {
               XCYBoolean.bulkRead();
               drive.simpleMovePeriod();
            }
         }
      }
   }
}
